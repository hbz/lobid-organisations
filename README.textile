...
Currently transforms two data sets from Pica-XML and CSV to JSON-LD for Elasticsearch indexing. Data of two or more entries are merged if the DBS ID (INR) of the entries is identical. The resulting data is used to build an index in Elasticsearch.
...
* Preprocess Sigel data, use DBS ID as record ID; if no DBS ID is available, use ISIL
...
Finally, the data can be indexed in Elasticsearch using a separate Java class (only works hbz internally). The ID of an organisation is given as a URI (e.g., http://data.lobid.org/organisations/DE-9). However, when building up the index, the organisations are given the last bit of the URI only (e.g., DE-9). Thus, Elasticsearch-internally, the organisations can be accessed via their ISIL or Pseudo-ISIL.
...
* @cd ../../../../@
* @mvn exec:java -Dexec.mainClass="flow.Enrich"@

h2. Index

Run the indexing (only works hbz internally!):

* @mvn exec:java -Dexec.mainClass="flow.Index"@
