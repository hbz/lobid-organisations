...
Download the input data dumps (only works hbz internally!):
...
Query the resulting index (using http://data.lobid.org, the public GET-only access proxy for our ES server):

* @curl -XGET 'http://data.lobid.org/organisations/_search?q=name:hbz+OR+altLabel:hbz'; echo@
...
* @curl -XGET 'http://data.lobid.org/organisations/dbs/ZY571'; echo@
...
* @curl -XGET 'http://data.lobid.org/organisations/dbs/ZY571/_source'; echo@
