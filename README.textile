...
Currently transforms two data sets from Pica-XML and CSV to JSON-LD for Elasticsearch indexing. The resulting data is used to build an index in Elasticsearch.
...
Finally, the data can be indexed in Elasticsearch using a separate Java class (only works hbz internally).
