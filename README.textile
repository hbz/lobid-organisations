...
Download Elasticsearch and unzip it in the project root directory:
@wget https://download.elastic.co/elasticsearch/elasticsearch/elasticsearch-2.3.1.zip@
@unzip elasticsearch-2.3.1.zip@

Run Elasticsearch in daemon mode (output is logged to logs/elasticsearch.log), and record the process id:
...
To continue, leave the server running or restart it, and change back to the project root directory:

@cd ..@

You have to choose your Elasticsearch server before building the project. If you don't want to run on localhost (default) change the following line in class flow.Constants and rebuild:
...
Run the indexing and pass a minimum file size in bytes:
...
* @curl -XGET 'http://localhost:9200/organisations/_search?q=name:hbz+OR+altLabel:hbz'; echo@
...
* @curl -XGET 'http://localhost:9200/organisations/organisation/DE-9'; echo@
...
* @curl -XGET 'http://localhost:9200/organisations/organisation/DE-9/_source'; echo@
...
* @~/activator-1.3.9-minimal/activator run@
